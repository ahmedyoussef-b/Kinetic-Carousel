...
import ViewModeTabs from '@/components/schedule/TimetableDisplay/components/ViewModeTabs';
...
export default function ShuddlePage() {
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            console.log("💧 [ShuddlePageClient] Début de la récupération des données initiales.");
            const data = await fetchAllDataForWizard();
            dispatch(setInitialData(data));
            setIsDataLoading(false);
            console.log("✅ [ShuddlePageClient] Hydratation terminée.");
        };
    ...
    useEffect(() => {
        ...
        if (!isDataLoading && !selectedViewId && wizardData) {
    ...
    }, [viewMode, wizardData, selectedViewId, isDataLoading]);
     ...
     if (isDataLoading) {
        return (
            <div className="flex flex-col items-center justify-center h-[calc(100vh-200px)]">
                ...
                <p className="mt-4 text-muted-foreground">Chargement des données du planificateur...</p>
            </div>
        );
    }
